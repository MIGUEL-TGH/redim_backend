...
class TransparenciaSectionsService {
  private const TABLE = 'transparencia_sections';
  private const COLS = "id, block, title, description, sort_order, status";
  private const BLOCKS = ['convocatorias', 'informes'];
  ...
  private static function validate(array $data): void {
    if (in_array($data['task'], ['insert','update'], true)) {
      ...
      if (empty($p['block']) || !in_array($p['block'], self::BLOCKS, true)) {
  ...
  public static function getAllData(): array {
    try {
      $items = BaseModel::query("SELECT " . self::COLS . " FROM " . self::TABLE . " ORDER BY block ASC, sort_order ASC, id ASC", [], 'all');
  ...
  public static function getActiveData(): array {
    try {
      $items = BaseModel::query("SELECT " . self::COLS . " FROM " . self::TABLE . " WHERE status = ? ORDER BY block ASC, sort_order ASC, id ASC", [1], 'all');
      return array_map(fn($i) => self::mapItem($i), $items);
    } catch (Throwable $e) {
      throw new DatabaseException($e->getMessage());
    }
