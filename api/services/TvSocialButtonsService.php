...
class TvSocialButtonsService {
  private const TABLE = 'tv_social_buttons';

  private static function validate(array $data): void {
    if (in_array($data['task'], ['insert','update'], true)) {
      $p = $data['params'];
      if (empty($p['label']) || mb_strlen((string)$p['label']) > 100) {
        throw new ValidationException(['label' => 'Etiqueta requerida (máx. 100)']);
      }
  ...
  private static function getById(int $id): array {
    $item = BaseModel::query("SELECT id, label, url, icon, sort_order, status FROM " . self::TABLE . " WHERE id = ?", [$id], 'one');
  ...
  public static function getAllData(): array {
    try {
      $items = BaseModel::query("SELECT id, label, url, icon, sort_order, status FROM " . self::TABLE . " ORDER BY sort_order ASC, id ASC", [], 'all');
  ...
  public static function getActiveData(): array {
    try {
      $items = BaseModel::query("SELECT id, label, url, icon, sort_order, status FROM " . self::TABLE . " WHERE status = ? ORDER BY sort_order ASC, id ASC", [1], 'all');
      return array_map(fn($i) => self::mapItem($i), $items);
    } catch (Throwable $e) {
      throw new DatabaseException($e->getMessage());
    }
