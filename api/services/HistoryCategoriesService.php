...
class HistoryCategoriesService {
  private const TABLE = 'history_categories';
  private const BLOCKS = ['reportajes', 'profundizar'];
  ...
  private static function validate(array $data): void {
    if (in_array($data['task'], ['insert','update'], true)) {
      ...
      if (isset($p['block']) && !in_array($p['block'], self::BLOCKS, true)) {
        throw new ValidationException(['block' => 'Bloque inválido']);
      }
    }
  }
  ...
  private static function getById(int $id): array {
    $item = BaseModel::query("SELECT id, name, block, sort_order, status FROM " . self::TABLE . " WHERE id = ?", [$id], 'one');
  ...
  public static function getAllData(): array {
    try {
      $items = BaseModel::query("SELECT id, name, block, sort_order, status FROM " . self::TABLE . " ORDER BY block ASC, sort_order ASC, id ASC", [], 'all');
  ...
  public static function getActiveData(): array {
    try {
      $items = BaseModel::query("SELECT id, name, block, sort_order, status FROM " . self::TABLE . " WHERE status = 1 ORDER BY block ASC, sort_order ASC, id ASC", [], 'all');
      return array_map(fn($i) => self::mapItem($i), $items);
    } catch (Throwable $e) {
      throw new DatabaseException($e->getMessage());
    }
