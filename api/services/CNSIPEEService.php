...
class CNSIPEEService {
  private const TABLE = 'cnsipee';
  //--------------------private access-------------------------------------------
  ...
  private static function getById(int $id): array {
    $sql =
    " SELECT cn.id, cn.woman, cn.man, cn.state_id AS state_id, s.name AS state_name, cn.status
        FROM cnsipee cn
        LEFT JOIN `states` s ON cn.state_id = s.id
        -- ORDER BY cn.id ASC
      WHERE cn.id = ?
    ";
  ...
  private static function checkDuplicate($stateID, $excludeId = null) {
    $sql = "SELECT id FROM cnsipee WHERE (state_id = :state_id)";
  ...
  public static function getActiveData(): array {
    try {
      $sql =
      " SELECT cn.id, s.name AS name, cn.woman, cn.man, cn.state_id AS cve_ent
        FROM cnsipee cn
        LEFT JOIN `states` s ON cn.state_id = s.id
        WHERE cn.status = ?
        ORDER BY s.id ASC;
