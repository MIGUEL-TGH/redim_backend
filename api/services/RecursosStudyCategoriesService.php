...
class RecursosStudyCategoriesService {
  private const TABLE = 'recursos_study_categories';
  ...
  private static function getById(int $id): array {
    $item = BaseModel::query("SELECT id, name, sort_order, status FROM " . self::TABLE . " WHERE id = ?", [$id], 'one');
  ...
  public static function getAllData(): array {
    try {
      $items = BaseModel::query("SELECT id, name, sort_order, status FROM " . self::TABLE . " ORDER BY sort_order ASC, id ASC", [], 'all');
  ...
  public static function getActiveData(): array {
    try {
      $items = BaseModel::query("SELECT id, name, sort_order, status FROM " . self::TABLE . " WHERE status = ? ORDER BY sort_order ASC, id ASC", [1], 'all');
      return array_map(fn($i) => self::mapItem($i), $items);
    } catch (Throwable $e) {
      throw new DatabaseException($e->getMessage());
    }
