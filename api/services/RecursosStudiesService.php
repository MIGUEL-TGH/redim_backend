...
class RecursosStudiesService {
  private const TABLE = 'recursos_studies';
  ...
  private static function mapItem(array $i): array {
    return [
      ...
      'title' => $i['title'],
      'url' => $i['url'],
      'category_id' => isset($i['category_id']) ? (int) $i['category_id'] : null,
  ...
  private static function selectSql(string $where = ''): string {
    return "SELECT s.id, s.title, s.url, s.category_id, c.name AS category_name, s.sort_order, s.status
            FROM " . self::TABLE . " s
            LEFT JOIN recursos_study_categories c ON s.category_id = c.id
            {$where}
  ...
  private static function getById(int $id): array {
    $item = BaseModel::query(
      "SELECT s.id, s.title, s.url, s.category_id, c.name AS category_name, s.sort_order, s.status
       FROM " . self::TABLE . " s
       LEFT JOIN recursos_study_categories c ON s.category_id = c.id
       WHERE s.id = ?", [$id], 'one');
  ...
  public static function getActiveData(): array {
    try {
      $items = BaseModel::query(self::selectSql('WHERE s.status = 1'), [], 'all');
      return array_map(fn($i) => self::mapItem($i), $items);
    } catch (Throwable $e) {
      throw new DatabaseException($e->getMessage());
    }
