    ...
    self::handle(function () {
      ...
      switch ($type) {
        ...
        case 'download_pdf':
          ...
          AuthMiddleware::authorize('users', 'read-only');

          // Validar que el nombre no contenga caracteres raros para evitar Directory Traversal
          $filename = $_GET['file'] ?? '';
          if(empty($filename) || strpos($filename, '..') !== false) {
            throw new ApiException("Archivo no válido", 400);
          }

          $params = [ 'filename' => $filename];

          return UsersService::downloadPDF($params);
